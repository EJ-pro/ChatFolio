...
class JsonParser(BaseTreeSitterParser):
    def parse(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 1. package.json-specific analysis (NPM/JS ecosystem)
            ...
            # 2. General JSON (environment config, etc.)
            else:
                # Very large data may cause serialization issues; extract key fields or summarize
                parsed_data["metadata"] = {k: v for k, v in data.items() if isinstance(v, (str, int, bool))}

        except Exception as e:
            meta["error"] = f"JSON Error during parsing: {str(e)}"
